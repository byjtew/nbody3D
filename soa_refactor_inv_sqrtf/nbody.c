...
void move_particles(particles_t p, const f32 dt, u64 n) {
    ...
    for (u64 i = 0; i < n; i++) {
        ...
        for (u64 j = 0; j < i; j++) {
            ...
            const f32 d_3_over_2 = d_2 * sqrtf(d_2);  // 11
            // _mm_rsqrt_ss


            const f32 inv_d_3_over_2 = 1.0f / d_3_over_2;
            // Net force
            fx += dx * inv_d_3_over_2;  // 13
            fy += dy * inv_d_3_over_2;  // 15
            fz += dz * inv_d_3_over_2;  // 17
        }
        ...
        for (u64 j = i + 1; j < n; j++) {
            ...
            f32 dz = p.z[j] - pz_i;                                         // 3
            const f32 d_2 = (dx * dx) + (dy * dy) + (dz * dz) + softening;  // 9
            const f32 d_3_over_2 = powf(d_2, 3.0f / 2.0f);  // 11
            const f32 inv_d_3_over_2 = 1.0f / d_3_over_2;
            // Net force
            fx += dx * inv_d_3_over_2;  // 13
            fy += dy * inv_d_3_over_2;  // 15
