...
void move_particles(particles_t p, const f32 dt, u64 n) {
    ...
    for (u64 i = 0; i < n; i++) {
        ...
        for (u64 j = i + 1; j < n; j++) {
            ...
            const f32 d_3_over_2 = powf(d_2, 3.0 / 2.0);  // 11

            d_dx = dt * dx / d_3_over_2;
            d_dy = dt * dy / d_3_over_2;
            d_dz = dt * dz / d_3_over_2;
...
int main(int argc, char **argv) {
    ...
    for (u64 i = 0; i < steps; i++) {
        ...
        const f32 h1 = (f32)(n) * (f32)(n - 1) / 2.0;

        // GFLOPS
        const f32 h2 = (29.0 * h1 + 3.0 * (f32)n) * 1e-9;
